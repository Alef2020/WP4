...
for bolt in Bolt_lst:
    ...
    for Mat in Material_lst:\
        ...
        def get_design(params):#bcs the optimization function does not let me pass on const parameters I have to redefine this function every time :(
            w ,h, t2 = params 
...
def pull_through_check_data(params):
    ...
    for fastener in design_option.FastenerList:
        ...
        F_max  = min(F_1, F_2)
        for LC in fastener.load_cases:
            MS = F_1/abs(LC.Force_Y) -1, F_2/abs(LC.Force_Y)-1 
            data.append(MS)
    return (min(data))
...
def bearing_check_data(params):
    ...
    for fastener in design.FastenerList:
        for LC in fastener.load_cases:
            ...
            sigma_lug = P_i / (fastener.configuration.d_s * design.t_2)
            sigma_wall = P_i / (fastener.configuration.d_s * design.t_3) 
            MS = ((design.material_wall.stress_yield/sigma_wall - 1), (design.material.stress_yield/sigma_lug - 1))
            data.append(MS)
    return(min(data))
